...
# Load database
sym_des = pd.read_csv(r"C:\Users\routh\PycharmProjects\Minor project\datasets\symtoms_df.csv")
precautions = pd.read_csv(r"C:\Users\routh\PycharmProjects\Minor project\datasets\precautions_df.csv")
workout = pd.read_csv(r"C:\Users\routh\PycharmProjects\Minor project\datasets\workout_df.csv")
description = pd.read_csv(r"C:\Users\routh\PycharmProjects\Minor project\datasets\description (1).csv")
medications = pd.read_csv(r"C:\Users\routh\PycharmProjects\Minor project\datasets\medications.csv")
diets = pd.read_csv(r"C:\Users\routh\PycharmProjects\Minor project\datasets\diets.csv")

# Load model
svc = pickle.load(open(r"C:\Users\routh\PycharmProjects\Minor project\model\svc.pkl", 'rb'))
...
def helper(dis):
    ...
    pre = [col for col in pre.values[0]] if len(pre) > 0 else []
...
def predict():
    if request.method == 'POST':
        user_symptoms = request.form.getlist('symptoms')  # Get list of selected symptoms
        ...
        try:
            ...
            return render_template('index.html', symptoms_dict=symptoms_dict, predicted_disease=predicted_disease,
                                   dis_des=desc, dis_pre=pre, dis_med=med, dis_diet=die, dis_wrkout=wrkout)
        except KeyError:
            return render_template('index.html', symptoms_dict=symptoms_dict, error="Invalid symptom selected.")
...
@app.route('/Documentation & User Guidance')
def Documentation():
    return render_template('Documentation & User Guidance.html')
